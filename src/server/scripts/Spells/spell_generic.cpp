        ...
        switch (GetSpellInfo()->Id)
        {
            ...
            case SPELL_COPY_WEAPON_3_AURA:
            {
                ...
                if (Player* player = caster->ToPlayer())
                {
                    ...
                    {
                if (uint32 entry = sTransmogrification->GetFakeEntry(mainItem) || TransmogDisplayVendorMgr::GetFakeEntry(mainItem))
            ...
            case SPELL_COPY_OFFHAND_2_AURA:
            {
                ...
                if (Player* player = caster->ToPlayer())
                {
                    ...
                    {
                        if (uint32 entry = sTransmogrification->GetFakeEntry(offItem) || TransmogDisplayVendorMgr::GetFakeEntry(offItem))
                            target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, entry);
                        else
            ...
            case SPELL_COPY_RANGED_AURA:
            {
                ...
                if (Player* player = caster->ToPlayer())
                {
                    ...
                    {
                        if (uint32 entry = sTransmogrification->GetFakeEntry(rangedItem) || TransmogDisplayVendorMgr::GetFakeEntry(rangedItem))
                            target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, entry);
                        else
